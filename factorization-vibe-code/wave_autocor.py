...
def autocorrelation_rolling(signal: np.ndarray, top_k: int = 5, min_peak_ratio: float = 2.5):
    L = len(signal)
    signal_energy = np.sum(np.abs(signal) ** 2)
    baseline = 0
    scores = []

    dot = np.vdot(signal, signal)
    for d in range(1, L // 2):
        shifted = np.roll(signal, d)
        score = np.abs(np.vdot(signal, shifted))
...
def wave_autocorr_factor(N, a, max_depth):
    for depth in [max_depth, max_depth * 2]:
        ...
        peaks = autocorrelation_rolling(signal, top_k=5)
...
def test_wave_autocorr():
    print("🌊 Wave Interference Factorization via Phase Autocorrelation (Rolling Optimized)")
    print("=" * 64)
